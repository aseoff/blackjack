...
def simulate_hand():
    #Check for player BJ
    player_start_hand = copy.copy(player_hand)
    dealer_start_hand = copy.copy(dealer_hand)
    if sum(player_hand) == 21:
        if dealer_show_card == 11 and sum(dealer_hand) == 21:
            if sum(dealer_hand) == 21:
                #Dealer also has blackjack with ace showing --> 0 money
                print("PLAYER AND DEALER HAVE BLACKJACK")
                results.append('pu')
                #NEW
                hand_result = 'pu'
                return [player_hand, dealer_hand, dealer_show_card, player_hand, dealer_hand, 'pu', len(shuffled_deck)]
            else:
                #Dealer also has blackjack with ace showing --> 0 money
                ...
                #NEW
                hand_result = 'pbj'
                return [player_hand, dealer_hand, dealer_show_card, player_hand, dealer_hand, 'pbj', len(shuffled_deck)]
            ...
            results.append('pu')
            #NEW
            final_dealer_hands = dealer_hand
            hand_result = 'pu'
            return [player_start_hand, dealer_start_hand, dealer_show_card, player_hand, dealer_hand, 'pu', len(shuffled_deck)]
            ...
            #NEW
            hand_result = 'pbj'
            return [player_start_hand, dealer_start_hand, dealer_show_card, player_hand, dealer_hand, 'pbj', len(shuffled_deck)]
        ...
        results.append('d')
        #NEW
        final_player_hands = player_hand
        final_dealer_hands = dealer_hand
        hand_result = 'd'
        return [player_start_hand, dealer_start_hand, dealer_show_card, player_hand, dealer_hand, 'd', len(shuffled_deck)]
        ...
        if pair(player_hand):
            ...
            if split_strategy(dealer_show_card, split_card) == 'split':
                ...
                #Add stat tracker in split details
                ...
                split_results = []
                for hand in final_split_hands:
                    hand_result = record_hand_result(hand, dealer_final_hand, split_hands_dds[final_split_hands.index(hand)])
                    results.append(hand_result)
                    split_results.append(hand_result)

                #CHECKPOINT!
                return [player_start_hand, dealer_start_hand, dealer_show_card, final_split_hands, dealer_final_hand, split_results, len(shuffled_deck)] #SPLIT!!!
                ...
                #NEW
                hand_result = record_hand_result(player_final_hand, dealer_final_hand, dd)
                results.append(hand_result)
                return [player_start_hand, dealer_start_hand, dealer_show_card, player_final_hand, dealer_final_hand, hand_result, len(shuffled_deck)]
...
for x in range (0,10):
    ...
    for i in range(0,100):
        ...
        #new - starting hands stat tracker
        starting_player_hand = player_hand
        starting_dealer_hand = dealer_hand
        ...
        dealer_show_card = dealer_hand[0]
        hand_stat = simulate_hand()
        hand_stat.insert(0,hand_id)
        
        
        hand_stats.append(hand_stat)
...
# print(hand_stats)
    ...
    data = hand_stats
    df = pd.DataFrame(data, columns = ['hand_id', 'player_starting_hand','dealer_starting_hand','dealer_show_card','player_final_hand(s)','dealer_final_hand', 'result(s)', 'num_remaining_cards'])

    #print(df.head())

    df.to_csv(f'sample{x+1}.csv', index = False)
